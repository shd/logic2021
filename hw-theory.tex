...
\begin{enumerate}
\item Докажите, что пустое множество единственно.
...
\end{enumerate}
\item Определите формулу $\phi(x)$ для свойства <<$x$ --- конечный ординал>>. 
Покажите с помощью схемы аксиом выделения и аксиомы бесконечности, что ординал $\omega$ определён.
...
\end{enumerate}
\item Проверьте следующие равенства:
\begin{enumerate}
\item $\omega\cdot\overline{3} = \overline{3}\cdot\omega$
...
\section*{Задание 13. Аксиома выбора. Мощность множеств}
В данном задании вам может потребоваться использовать аксиому выбора или её альтернативную формулировку
(любое множество может быть вполне упорядочено, у любой сюрьективной функции существует частичная обратная,
лемма Цорна и т.п.).
\begin{enumerate}
\item Покажите, что следующие множества имеют счётную мощность: $\omega\cdot\omega$, $\omega^\omega$.
...
\item Если существует функция $f: a \rightarrow b$ на вполне упорядоченных множествах $a$ и $b$, 
сохраняющая порядок (если $x < y$, то $f(x) < f(y)$), то либо она --- биекция, либо найдётся 
такой элемент $x \in b$, что $\{f(i)|i \in a\} = \{t \in b | t < x\}$.
\item Покажите, что если $a$ и $b$ --- множества, то $|a| \le |b| \vee |b| \le |a|$.
\item Покажите, что для любого множества $a$ определено его кардинальное число $|a|$.
\end{enumerate}
